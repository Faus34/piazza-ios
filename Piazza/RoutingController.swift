...
class RoutingController: BaseNavigationController, PathDirectable {
    private enum PresentationType: String { case advance, replace, modal }
...
extension RoutingController: SessionDelegate {
    ...
    func session(_ session: Session, didFailRequestForVisitable visitable: Visitable, error: Error) {
        if let turboError = error as? TurboError, turboError == .http(statusCode: 401) {
            ...
            alert.addAction(UIAlertAction( title: NSLocalizedString("error.ok", comment: ""),style: .default,handler: nil ))
    ...
    func visit(_ proposal: VisitProposal) {
        ...
        let viewController = ViewControllerVendor.viewController( for: proposal.url, properties: proposal.properties )
    ...
    private func navigateTo(_ vc: UIViewController, using presentationType: PresentationType){
        ...
        case .modal:
            ...
            if let presentedViewController = presentedViewController {
                presentedViewController.dismiss( animated: true, completion: { [unowned self] in self.present(modalNavController, animated: true) })
            } else {
